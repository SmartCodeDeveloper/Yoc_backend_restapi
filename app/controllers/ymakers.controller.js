const Ymakers = require("../models/ymakers.model.js");
...
exports.create = (req, res) => {
    ...
    Ymakers.create(ymakers, (err, data) => {
      if (err)
        res.status(500).send({
          message:
...
exports.findOne = (req, res) => {
  Ymakers.findByAddress(req.params.address, (err, data) => {
    if (err) {
...
exports.update = (req, res) => {
  ...
  Ymakers.updateByAddress(
    new Ymakers(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
...
exports.delete = (req, res) => {
  Ymakers.remove(req.params.address, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found project with id ${req.params.address}.`

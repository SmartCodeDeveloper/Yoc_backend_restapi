const Yfinancial = require("../models/yfinancial.model.js");
...
exports.create = (req, res) => {
    ...
    // Create a Tutorial
    const yfinancial = new Yfinancial({
    ...
    Yfinancial.create(yfinancial, (err, data) => {
      if (err)
        res.status(500).send({
          message:
...
exports.findOne = (req, res) => {
  Yfinancial.findByAddress(req.params.address, (err, data) => {
    if (err) {
...
exports.update = (req, res) => {
  ...
  Yfinancial.updateByAddress(
    new Yfinancial(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
...
exports.delete = (req, res) => {
  Yfinancial.remove(req.params.address, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found project with id ${req.params.address}.`

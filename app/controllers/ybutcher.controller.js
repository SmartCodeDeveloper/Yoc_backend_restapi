const Ybutcher = require("../models/ybutcher.model.js");
...
exports.create = (req, res) => {
    ...
    Ybutcher.create(ybutcher, (err, data) => {
      if (err)
        res.status(500).send({
          message:
...
exports.findOne = (req, res) => {
  Ybutcher.findByAddress(req.params.address, (err, data) => {
    if (err) {
...
exports.update = (req, res) => {
  ...
  Ybutcher.updateByAddress(
    new Ybutcher(req.body),
    (err, data) => {
      if (err) {
        if (err.kind === "not_found") {
          res.status(404).send({
...
exports.delete = (req, res) => {
  Ybutcher.remove(req.params.address, (err, data) => {
    if (err) {
      if (err.kind === "not_found") {
        res.status(404).send({
          message: `Not found project with id ${req.params.address}.`
